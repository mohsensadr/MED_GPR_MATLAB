...
MAX = 50;                         % max 20 function evaluations per line search
...
if length>0, S='Linesearch'; else S='Fun. ev.'; end 

i = 0;                                            % zero the run length counter
ls_failed = 0;                             % no previous line search has failed
[f0 df0] = feval(f, X, varargin{:});          % get function value and gradient
Z = X; X = unwrap(X); df0 = unwrap(df0);
fprintf('%s %3i;  Value %2.2e\r', S, i, f0);
...
while i < abs(length)                                      % while not finished
  ...
  while 1                             % keep extrapolating as long as necessary
    ...
    while ~success && M > 0
      try
        ...
        success = 1;
        %fprintf('%s %3i;  Value %2.2e, df=%2.2e\r', S, i, f3, norm(df3));
  ...
  if abs(d3) < -SIG*d0 && f3 < f0+x3*RHO*d0          % if line search succeeded
    ...
    fprintf('line search succeeded, %s %6i;  Value %4.6e\r', S, i, f0);
    if exist('fflush','builtin') fflush(stdout); end
